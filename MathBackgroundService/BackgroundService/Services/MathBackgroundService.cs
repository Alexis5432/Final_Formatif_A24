        ...
        foreach (var userId in _data.Keys)
        {
            ...
            if (userData.Choice == _currentQuestion!.RightAnswerIndex)
            {
                    ...
                    if (player != null)
                    {
                        ...
                        await _mathQuestionHub.Clients.User(userId).SendAsync("RightAnswers");
                    }
            }
            else
            {
                    await _mathQuestionHub.Clients.User(userId).SendAsync("WrongAnswers", _currentQuestion.Answers[_currentQuestion.RightAnswerIndex]);
            }
